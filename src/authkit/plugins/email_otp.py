...
The actual flow logic lives in :mod:`authkit.flows.email_otp` so the
...
from authkit.flows.credentials import EmptyResponse, SessionResponse
from authkit.flows.email_otp import (
...
from authkit.security.otp import OtpService
...
class EmailOtpPlugin(Plugin):
    ...
    id: ClassVar[str] = "authkit-email-otp"
    ...
    def client_ip(self, request: Request) -> str | None:
        from authkit.web.fastapi import client_ip
    ...
    async def sign_in_handler(
        ...
        from authkit.web.fastapi import set_session_cookie
