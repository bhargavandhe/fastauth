...
First concrete ``Plugin`` subclass contributing endpoints to the AuthKit router.
...
from authkit.domain.events import ApiKeyCreated, ApiKeyRevoked, ApiKeyVerifyFailed
from authkit.domain.models import ApiKey, WireModel
from authkit.exceptions import ConfigError, InvalidCredentialsError, NotFoundError
...
class ApiKeyPlugin(Plugin):
    ...
    id: ClassVar[str] = "authkit-api-key"
    ...
    def bind(self, context: AuthContext) -> None:
        """Attach the assembled ``AuthContext``. Called by ``AuthKit.__init__``."""
    ...
    async def current_user_id(self, request: Request) -> str:
        ...
        ``authkit.web.fastapi`` and ``authkit.plugins.api_key``.
        """
