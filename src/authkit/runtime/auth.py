"""AuthKit — main entrypoint composing every authkit subsystem."""
...
from authkit.plugins.base import Plugin, PluginRegistry
from authkit.runtime.api import AuthApi
...
from authkit.security.passwords import Argon2idHasher, PasswordHasher
...
class AuthKit:
        ...
        *,
        ...
        if session_strategy is None:
            if config.session.strategy is SessionStrategyKind.DATABASE:
                ...
                from authkit.security.jwt import JwtSessionStrategy
    ...
    def as_asgi(self) -> FastAPI:
        """Return a standalone ``FastAPI`` app wrapping the authkit router."""
        app = FastAPI(title="authkit", lifespan=self.lifespan)
        self.install(app)
        return app

    def install(self, app: FastAPI) -> None:
        """Install authkit routes and middleware on an existing ``FastAPI`` app.

        This is the class-based convenience path for host-app integration:
        construct ``AuthKit`` explicitly, create your ``FastAPI`` app with the
    ...
    #
    # These are bound methods on the ``AuthKit`` instance so they capture the
