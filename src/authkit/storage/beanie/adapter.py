...
domain layer (:mod:`authkit.domain.models`) keeps these as ``str`` for storage
...
from authkit.exceptions import DuplicateError, NotFoundError
from authkit.storage.beanie.documents import (
    AccountDoc,
    ApiKeyDoc,
    AuditLogDoc,
    JwksKeyDoc,
...
from authkit.storage.beanie.helpers import (
    normalise_datetimes,
...
class BeanieAdapter:
    ...
    :mod:`authkit.domain.models` instances stay storage-agnostic strings.
    """
    ...
    def lifespan(self, auth: AuthKit) -> Callable[[FastAPI], AbstractAsyncContextManager[None]]:
        """Return a FastAPI lifespan that initialises Beanie, then authkit.
        ...
        preserving ``AuthKit`` as storage-agnostic runtime code.
        """
