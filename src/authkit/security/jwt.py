...
from authkit.domain.models import JwksKey, Session, User
from authkit.exceptions import JwksDecryptionError
from authkit.security.sessions import SessionContext
from authkit.storage.base import JwksKeyStore, UserStore

LOGGER = logging.getLogger("authkit.jwt")
...
class JwksRegistry:
    ...
    async def ensure_key(self) -> JwksKey:
        ...
        ``AuthKitConfig.secret_key`` changed without an accompanying
        ...
        for candidate in active:
            try:
                ...
                LOGGER.warning(
                    ...
                    "the current AuthKitConfig.secret_key. "
    ...
    def decrypt_private_jwk(self, key: JwksKey) -> dict[str, Any]:
        ...
        ``AuthKitConfig.secret_key`` is different from the secret that encrypted
            ...
            if plain is None:
                raise JwksDecryptionError(
                    message=(
                        ...
                        "AuthKitConfig.secret_key was changed without adding the previous "
