...
from authkit.runtime.api import AuthApi, HealthResponse
...
class CamelJSONResponse(JSONResponse):
    ...
    ``AuthKitConfig.wire_format`` is :class:`WireFormat.CAMEL`. SNAKE
...
class AuthKitRoute(APIRoute):
    """Custom route class that converts ``AuthKitError`` into a JSON response.
...
def build_router(context: AuthContext, api: AuthApi) -> APIRouter:
    """Build the authkit ``APIRouter`` with health + credentials flow endpoints."""
...
def install_csrf(app: FastAPI, context: AuthContext) -> None:
    ...
    Use this when integrating authkit via ``app.include_router(auth.router)``
    on your own ``FastAPI`` application. ``AuthKit.as_asgi()`` already installs
...
def install_security_headers(app: FastAPI, context: AuthContext) -> None:
    ...
    response. ``AuthKit.as_asgi()`` already installs this on the standalone
