"""In-process pub/sub bus for :class:`fastauth.domain.events.AuthEvent`.

The bus is the **runtime** half of fastauth's event system; the event data
classes themselves live in :mod:`fastauth.domain.events` since they're pure
data with no orchestration. Plugins subscribe handlers at construction
time via :meth:`FastAuth.__init__`; flows publish events as side effects.
"""
...
LOGGER = logging.getLogger("fastauth.events")
